...
def detectarBordes(imagenCelula, output="output.png", nivelUmbral=125):
    ...
    pic, puntos_borde = umbralFiltro(imagenCelula, nivelUmbral=nivelUmbral)
...
def detectarCelulasBasofilo(nombreImagen, output="analisisBasofilo.png", size=(128, 128)):
    image = Image.open(nombreImagen)
    original_image = image.copy()

    image.thumbnail(size, Image.ANTIALIAS)

    border_points, gradient_values = detectarBordes(image, output="output.png", nivelUmbral=60)
    imageEllipse = buscarCentros(image, border_points, gradient_values, output)
    imageEllipse.save('grayscale_'+output)
    umbralFiltro(imageEllipse, nivelUmbral = 130)
    ...
    for i in range(len(centers)):
        ...
        global numeroCelulasEncontradas 
        numeroCelulasEncontradas = i + 1 
...
def detectarCelulasLinfocito(nombreImagen, output, base_url, size=(128, 128)):
    image = Image.open(nombreImagen)
    original_image = image.copy()
    ...
    border_points, gradient_values = detectarBordes(image, output="output.png", nivelUmbral=60)
    imageEllipse = buscarCentros(image, border_points, gradient_values, output)
    imageEllipse.save(base_url + 'grayscale_'+ "resultado2.png")
    umbralFiltro(imageEllipse, nivelUmbral = 130)
    centers = detectarCentro(imageEllipse)
...
def basofilosColor():
    ...
    for (lower, upper) in boundaries:
        ...
        cv2.imwrite("/Users/gustavo/Desktop/TTA050/BasofiloAnalizar.jpg", output)
        break

def linfocitosColor(image, url):
    output = Image.new("L", (1200, 1200))
    while(1):
        lower = np.array([139,0,139], dtype = "uint8")
        upper = np.array([214,112,218], dtype = "uint8")
        mask = cv2.inRange(image, lower, upper)
        output = cv2.bitwise_and(image, image, mask = mask)
        print output
...
def analize_image(img):
    base_url = BASE_DIR + "/img/results/"
    url1 = base_url + "resultado1.jpg"
    url2 = base_url + "resultado2.jpg"
    image = cv2.imread(img)
    linfocitosColor(image=image, url=url1)
    detectarCelulasLinfocito(url1, url2, base_url)

#def main():
 #   a = sys.argv[2]
  #  if a == "/Users/eric/Desktop/TTA050/Linfocitos/l5.jpg":
   #     linfocitosColor()
    #else:
     #   basofilosColor()
